...
public class PathfindingGraph {
    ...
    public void InitializeGraph(LevelObject levelObject) {
        nodes = new PathfindingNode[levelObject.blocks.Length];
        ...
        for (int i = 0; i < nodes.Length; i++) {
            ...
            if (levelObject.blockObjects[i] == null) continue;
            node.obj = levelObject.blockObjects[i];
    ...
    public void InitializeNode(PathfindingNode node, int i) {
        ...
        PathfindingRules nodeRules = node.obj.GetComponent<PathfindingRules>();
        ...
        if ((i % xSize) + 1 < xSize && nodes[i + 1] != null) {
            ...
            otherRules = other.obj.GetComponent<PathfindingRules>();
        ...
        if ((i % xSize) + 1 < xSize && (i / xSize / ySize) + 1 < zSize && nodes[i + 1 + xSize * ySize] != null) {
            ...
            if (other2 == null) {
                ...
                PathfindingRules otherRules2 = other2.obj.GetComponent<PathfindingRules>();
    ...
    public void UpdateNode(Vector3Int position, GameObject newObject) {
        int index = position.x + position.y * xSize + position.z * xSize * ySize;
        PathfindingNode node = nodes[index];
        if (node == null) {
    ...
    public void SwapNodes(Vector3Int location1, Vector3Int location2) {
        ...
        Vector3 tempLoc = temp1.location;
    ...
    public void MoveNode(Vector3Int location1, Vector3Int location2, GameObject obj1, GameObject obj2) {
        ...
        UpdateNode(location1, obj2);
        UpdateNode(location2, obj1);
    }
    ...
    public void CreateNode(Vector3Int location) {
        ...
        node.obj = LevelRenderer.instance.GetObject(location);
        node.incoming = new List<PathfindingNode>();
        node.outgoing = new List<PathfindingNode>();
        node.adjacent = new List<PathfindingNode>();
